...
//This function provides the view for searching the houses by longitude and latitude
export default function ByLL() {
  const [lat, setLat] = useState("");
  const [long, setLong] = useState("");

	//updates the list of houses when the user gives input
  const updateLat = async (lat) => {
    setLat(lat);
  };
  const updateLong = async (long) => {
    setLong(long);
  };

  const [houses, setHouses] = useState([]);

  async function getHouses(long, lat) {
    let response = null;
    if (long === "" || lat === "") {
      response = await fetch(`http://localhost:6942/properties/`);
    } else {
      response = await fetch(
        `http://localhost:6942/properties/byCoord/${long}/${lat}`
      );
    }
  ...
  return (
    <div>
      <h3>Search for property by longitude and latitude</h3>
      <br></br>
      <SearchBar input={long} onChange={updateLong} value={"longitude"} />
      <br></br>
      <br></br>
      <SearchBar input={lat} onChange={updateLat} value={"latitude"} />
      <Link className="btn btn-link" to={`/updateList/${long}/${lat}`}>
        Edit for the whole list
      </Link>
